...
public class UserController {
    ...
    public ResponseEntity<ApiResponse<MyPage>> getUserMyPage(@AuthenticationPrincipal AuthMember authMember) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.onSuccess(GeneralSuccessCode.OK, userService.getUserMyPage(authMember)));
    }

    @PostMapping("/api/auth/signup")
    public ResponseEntity<ApiResponse<Long>> signup(@RequestBody SignupUser user) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.onSuccess(UserSuccessCode.USER_CREATED, userService.signup(user)));
    }

    @PostMapping("/api/login")
    public ResponseEntity<ApiResponse<LoginResponse>> login(@RequestBody LoginUser user) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(ApiResponse.onSuccess(GeneralSuccessCode.OK, userService.login(user)));
    }}
